...
**Current state:** `platform-engineering-talk.md` is the deck — 35 slides, ~3.9 MB (13 base64 photographs + 5 embedded diagrams + 3 base64 logos), exported alongside as `.html` and `.pdf`. It is titled **"Platform Thinking in the Real World: People, Process & Mindset"**, the title announced to the meetup group; *"ahead of the scale"* survives inside the deck as the thesis line, not as the cover. It is themed to the **ProductTank 2026 template** (see below). `platform-engineering-talk-prep.md` is the speaker's prep companion — the red-flag myth correction, anticipated pushback, the verification ledger, and delivery notes all live there, **not on slides**; prep material removed from the deck goes into that file, and slide-facing `.cite` lines carry clean sources only (no "CONFIRMED/CORRECTED" chatter). Wave artifacts live in `docs/feature/platform-engineering-talk/`, research in `docs/research/`, and the image pipeline in `tools/`. The repo is now under version control and pushed to [github.com/v1bh0r/pt-bengaluru-talk-aug-2026](https://github.com/v1bh0r/pt-bengaluru-talk-aug-2026); there is still no `package.json` — do not add one unless the author asks.
...
Eight Excalidraw diagrams exist as sources; **five are embedded**: the platform cycle (closed loop), the sign-shape hierarchy, the scaled lag timeline, the strangler-fig/FHIR architecture, and the org before/after (`08-org-boundaries` — activity-sliced teams each repeating five foundation bars, collapsing into one platform band under domain-sliced teams; it deliberately reuses `04-coordination`'s before/after grammar and the diagnosis slide's five words). The Roman road cross-section, McAdam vs Telford, and the coordination-cost before/after (`04-coordination`, cut when its slide "A line painted on a road" merged into "Then, in twelve years" — the merged slide already carries the sign-shapes diagram) were cut from the deck as material a product audience does not need on screen, but their sources stay in `assets/diagrams/` in case the talk is re-lengthened for a 60-minute slot.
...
Two brand marks are embedded as base64 in the style block and on the cover: the generic **ProductTank** wordmark (dark + white variants, sourced from `~/Documents/product-tank-chd.github.io/assets/logos/` — that repo's origin, not a claim about which chapter is hosting) and the **Trantor** logo (SVG data URI, from `~/Documents/Trantor-Logo_Color.svg`). The wordmark image carries no city; the adjacent `.foot-chapter` text badge is the only chapter-specific part of the footer and **must name the host chapter of the event**, not the speaker's own chapter — for this deck that's Bengaluru, even though the speaker's own bio (`.sp-role`, elsewhere on the cover) correctly credits him as an organiser of ProductTank Chandigarh. Don't conflate the two. The Trantor mark is teal `#388DA9` on grey `#90918C` — outside the ProductTank palette by necessity, so it only ever appears on the cover's white footer bar, never on navy.
...
| `visual` | Full-bleed photograph — `.vbg` + `.bg-<slug>` background, `.vcap` caption with a `.vyear` blue chip. A pure beat, 6–8 seconds on stage. Seven remain. |
| `hero` | A photo beat **merged with the content slide that followed it** — `.hbg` + `.bg-<slug>` band across the top carrying `.vyear` + `h1`, then `.hbody` with the usual white-slide content below. Six of these. The band takes every pixel the content leaves, down to a `min-height: 38%` floor, so a light body shows more photograph instead of stranding white at the foot of the slide. Three things that are easy to get wrong: the `.bg-*` scrim is a full-slide gradient, so `background-size: 100% 240px` clamps it to a band at the photo's foot (otherwise it swallows 70% of the image); `.hbg::before` carries that gradient's terminal 4% tint over the rest of the photo, or the clamp leaves a visible seam; and `.hbody` is bottom-anchored, so its `padding-bottom` must clear the absolutely-positioned `.cite`. A portrait may need a per-slide `background-position` override (see `.bg-bertha`). Suppresses `.era`, because the `.vyear` chip on the band replaces it. **`hero flip`** reverses it — content on top, photograph at the foot — for a beat that has to be the last thing on screen; it takes the white wordmark and light pagination, needs `.cite.inline` (the absolute `.cite` would land on the photo), and stacks a second gradient under its caption because the baked-in scrim only reaches full strength at the very bottom edge. |
...
When forcing a count, **never invent a fact to reach five.** Merge instead, or derive the new item from figures already on the slide (the interchange slide's `1.8×` is the ratio of the two published FHWA figures, and its `.cite` says so). Reaching five by combining is always safe; reaching five by inventing is never.
...
`section.visual` uses `.vbg` and `section.hero` uses `.hbg`, both with a `.bg-<slug>` class — the cover is photo-free. The scrim is `rgba(13,2,40,…)` — a *darkened* navy, not `#17044a` itself. Using the brand navy at low opacity casts the photographs magenta; the darkened value keeps the family without the tint. Thirteen `.bg-*` rules sit at the end of the style block, one per photograph, and each is ~250–450 KB — if a photo slide is deleted, delete its rule too, or the file carries dead weight.
